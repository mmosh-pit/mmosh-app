...
export class BaseSpl {
  ...
  async __getCreateTokenInstructions(opts: createTokenOptions) {
    ...
    const rent =
      await this.__connection.getMinimumBalanceForRentExemption(MINT_SIZE);
  ...
  async mintToken(
    ...
    this.__reinit();
    let { mint, authority, payer, receiver, amount, decimal } = input;
    ...
    const { ata, ix: initAtaIx } =
      await this.__getOrCreateTokenAccountInstruction({
        payer,
        mint,
        owner: receiver,
      });
    ...
    const ix = createMintToInstruction(mint, ata, authority, amount);
