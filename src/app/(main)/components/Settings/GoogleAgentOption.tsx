...
const GoogleAgentOption = () => {
  ...
  const connectGoogleAccount = React.useCallback(() => {
    ...
    window.location.href = "/api/auth/google/start?user=" + userData!.ID;
  ...
  const fetchGoogleStatus = async () => {
    try {
      const res = await internalClient("/api/google/status?user=" + userData!.ID);
  ...
  const removeGoogleAccount = React.useCallback(() => {
    ...
    console.log('Removing Google Account...');
    internalClient.delete('/api/google/status').then(() => {
      ...
      console.log("Error removing Google account:", error);
    });
  }, []);
