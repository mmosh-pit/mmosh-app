...
import {
  pinFileToShadowDrive,
  pinFileToShadowDriveWithFileName,
} from "@/app/lib/uploadFileToShdwDrive";
...
export const GET = async (req: Request) => {
  try {
    ...
    let referer = requestUrl.searchParams.get("referer")
      ? requestUrl.searchParams.get("referer")
      : projectInfo.data.project.key;
    ...
    const baseHref =
      ...
      `/api/actions/guest-pass?referer=${referer}`;
...
export const POST = async (req: Request) => {
  try {
    ...
    let projectConn: ProjectConn = new ProjectConn(
      ...
      new anchor.web3.PublicKey(projectInfo.data.project.key),
    );
    console.log("test6");
        ...
        if (metadata) {
          if (metadata.project == projectInfo.data.project.key) {
            let actionError: ActionError = {
              message: "User already have pass minted in his account",
            };
            return Response.json(actionError, {
              status: 400,
              headers,
    ...
    const metaBody = {
      ...
      enternal_url:
        process.env.NEXT_PUBLIC_APP_MAIN_URL +
        "create/project/" +
        projectInfo.data.project.symbol,
      family: "MMOSH",
    ...
    const passMetaURI: any = await pinFileToShadowDriveWithFileName(
      metaBody,
      body.account + "_" + type,
    );
