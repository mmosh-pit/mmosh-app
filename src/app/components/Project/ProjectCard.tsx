...
const ProjectCard = (projectData: any) => {
  ...
  useEffect(() => {
    ...
    if (projectData) {
      ...
      let dexListDate = convertUTCDateToLocalDate(
        new Date(projectData.data.dexlistingdate),
      );
  ...
  return (
    <>
      {projectData && (
        <div
          className="project-card relative rounded-md overflow-hidden cursor-pointer"
          onClick={projectData.onClick}
        >
          <img
          ...
          <div className="absolute project-card-background rounded-md overflow-hidden bottom-2 left-2 right-2 px-2.5 py-3.5">
            <div className="project-card-head grid grid-cols-12 gap-3">
              <div
                className={
                  projectData.showTimer
                    ? "project-card-head-left  col-span-4"
                    : "project-card-head-left  col-span-12"
                }
              >
                <h4 className="text-lg text-white leading-3 capitalize text-ellipsis whitespace-nowrap overflow-hidden">
                  {projectData.data.name}
                </h4>
                <p className="text-tiny text-gray-300 inline-block capitalize underline">
              ...
              {countDown != 0 && projectData.showTimer && (
                <div className="col-span-8 flex flex-1 justify-end">
                  <div className="project-card-head-right col-span-8 flex">
                    <p className="text-tiny text-gray-300 text-ellipsis whitespace-nowrap overflow-hidden">
                      Time to
                      <br /> Listing
                    </p>
                    <div className="timer flex">
                      <div className="timer-item ml-1 text-center bg-black bg-opacity-[0.56] p-1.5 rounded-md">
                        <label className="block text-lg text-white font-bold leading-none">
                          {" "}
                          {minTwoDigits(getCountDownValues(countDown).days)}
                        </label>
                        <span className="block text-[6px] text-gray-300 leading-none">
                          Days
                        </span>
                      </div>
                      <div className="timer-item ml-1 text-center bg-black bg-opacity-[0.56] p-1.5 rounded-md">
              ...
              {countDown == 0 && (
                <div className="col-span-8 flex flex-1 justify-end project-card-graph">
                  <div>
                    <h4 className="text-base text-white leading-4 capitalize text-ellipsis whitespace-nowrap overflow-hidden">
                      {projectData.data.coins[0].name}
                    </h4>
                    <p className="text-tiny text-gray-300 inline-block capitalize">
                      {projectData.data.coins[0].symbol}
                    </p>
                  </div>
                  {projectData.data.coins[0].prices.length > 0 && (
                    <div className="text-right pl-3.5">
                      ...
                      {projectData.data.coins[0].pricepercentage < 0 && (
                        <p className="text-small-font-size flex justify-end crate-down-container">
                          <CaretDown />
                          <span className="ml-1">
                            {projectData.data.coins[0].pricepercentage}%
                          </span>
                        </p>
                      )}

                      {projectData.data.coins[0].pricepercentage >= 0 && (
                        <p className="text-small-font-size flex justify-end crate-up-container">
                          <CaretUp />
                          <span className="ml-1">
                            {projectData.data.coins[0].pricepercentage}%
                          </span>
            ...
            <div className="project-card-details grid grid-cols-2 gap-3">
              <div className="project-card-detail-item flex">
                <label className="text-xs text-white pr-1.5 leading-none">
                  Listing Price
                </label>
                <span className="text-xs text-gray-300 leading-none">
                  ${projectData.data.coins[0].listingprice.toLocaleString()}
                </span>
              </div>
              <div className="project-card-detail-item flex">
                <label className="text-xs text-white pr-1.5 leading-none">
                ...
                <span className="text-xs text-gray-300 leading-none overflow-hidden text-ellipsis">
                  {projectData.data.coins[0].supply.toLocaleString()}
                </span>
              </div>
              {countDown != 0 && (
                <div className="project-card-detail-item flex">
              ...
              <div className="project-card-detail-item flex">
                ...
                <span className="text-xs text-gray-300 leading-none overflow-hidden text-ellipsis">
                  {(
                    projectData.data.coins[0].listingprice *
                    projectData.data.coins[0].supply
                  ).toLocaleString()}{" "}
                  <label className="text-tiny text-white font-bold">USDC</label>
                </span>
              </div>
            </div>
          </div>
