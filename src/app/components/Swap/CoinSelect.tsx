...
const CoinSelect = ({
  ...
  const getCoinsList = async() => {
    ...
    if(otherToken?.token === "") {
      ...
      let memeCoin = listResult.data.filter((item: any) => item.token == otherToken?.token);
      ...
      if(otherToken?.symbol.toLowerCase() == "ptvb" || otherToken?.symbol.toLowerCase() == "ptvr") {
        newCoinList.push(jupCoins[0]) // mmosh
      }
      ...
      if(otherToken?.symbol.toLowerCase() == "mmosh" || otherToken?.symbol.toLowerCase() == "ptvr" || otherToken?.symbol.toLowerCase() == "ptvb") {
        for (let index = 0; index < listResult.data.length; index++) {
          const element = listResult.data[index];
          element.decimals = 9
          element.iscoin = false
  ...
  const handleTokenSelect = (token: Coin) => {
    ...
    (document.getElementById(isBase ? "coin_modal_base" : "coin_modal_target") as any)?.close();
  }
  ...
  const onOpenModel = async () => {
     await getCoinsList();
     (document.getElementById(isBase ? "coin_modal_base" : "coin_modal_target") as any)?.showModal()
  }
  ...
  return (
    <>
      ...
      <dialog id={isBase ? "coin_modal_base" : "coin_modal_target"} className="modal">
