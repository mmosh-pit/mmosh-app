...
const LaunchPad = () => {
    ...
    useEffect(()=>{
        if(latest) {
             let dexListDate = convertUTCDateToLocalDate(new Date(latest.dexlistingdate))
        ...
        <div className="relative background-content">
            <div className="flex flex-col items-center justify-center w-full py-12">
                <div className="relative w-full flex flex-col justify-center items-center">
                    <h2 className="text-center text-white font-goudy font-normal text-xl">Launchpad</h2>
                </div>
            </div>
                    ...
                    <div className="search-container md:flex p-10">
                        ...
                        {latest && !projectLoading &&
                            <div className="w-[300px] pt-10 md:pt-0 mx-auto">
                                <ProjectCard data={latest} onClick={()=>{onProjectSelect(latest)}} showTimer={false} />
                            </div>
                        }
                ...
                <div className="pt-5 pb-12 border-t border-white border-opacity-20">
                   <div className="relative w-full flex flex-col justify-center items-center">
                        <div className="relative flex search-container">
                            ...
                            <input type="text" placeholder="Type your search terms" className="input bg-transparent px-2.5 h-10 border-0 focus:outline-0 text-xs w-52" onChange={(event) => searchProject(event.target.value)} />
                        </div>
                    </div>
                </div>
                ...
                {projects.length > 0 && !projectLoading &&
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                        {projects.map((projectItem: any, index: any) => (
                            <ProjectCard data={projectItem} onClick={()=>{onProjectSelect(projectItem)}} showTimer={true}/>
                        ))}
                    </div>
                }
                {projectLoading &&
